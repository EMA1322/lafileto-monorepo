...
async function loadCategories() {
  if (DATA_SOURCE === 'api') {
    ...
    if (!res?.ok)
      throw res?.error || { code: 'INTERNAL_ERROR', message: 'No se pudieron cargar categorías' };
    return Array.isArray(res.data)
      ? res.data.map((c) => ({ id: c.id, name: c.name, status: c.status }))
      : [];
  }
  // JSON
...
async function loadProducts() {
  if (DATA_SOURCE === 'api') {
    ...
    if (!res?.ok)
      throw res?.error || { code: 'INTERNAL_ERROR', message: 'Error al listar productos' };
    return {
      rows: res.data || [],
      meta: res.meta || {
        page: state.page,
...
async function openEditModal(id, product) {
  ...
  const categoryOptions = categoriesCache
    ...
    .map(
      (c) =>
        `<option value="${escapeAttr(c.id)}" ${String(c.id) === String(product.categoryId) ? 'selected' : ''}>${escapeHtml(c.name)}</option>`,
    )
    .join('');
  ...
  $form.addEventListener('submit', async (e) => {
    ...
    if (!payload.name) {
      showSnackbar('Revisá los datos: hay campos inválidos.', {
        type: 'error',
    ...
    if (!payload.categoryId) {
      showSnackbar('Seleccioná una categoría válida.', { type: 'error', code: 'VALIDATION_ERROR' });
      return;
    }
    if (!(payload.price >= 0)) {
      showSnackbar('El precio debe ser un número válido.', {
        type: 'error',
        code: 'VALIDATION_ERROR',
      });
      return;
    ...
    if (payload.isOffer && (payload.discount < 0 || payload.discount > 95)) {
      showSnackbar('El descuento debe ser entre 0 y 95%.', {
        type: 'error',
        code: 'VALIDATION_ERROR',
      });
      return;
    ...
    try {
      if (DATA_SOURCE === 'api') {
        ...
        if (!res?.ok)
          throw res?.error || { code: 'INTERNAL_ERROR', message: 'No se pudo guardar el producto' };
      ...
      const msg =
        code === 'VALIDATION_ERROR'
          ? 'Revisá los datos: hay campos inválidos.'
          : code === 'AUTH_REQUIRED' || code === 'AUTH_INVALID'
            ? 'Iniciá sesión / Tu sesión expiró.'
            : code === 'RATE_LIMITED'
              ? 'Demasiadas solicitudes. Probá en unos minutos.'
  ...
  const onConfirm = async () => {
    try {
      if (DATA_SOURCE === 'api') {
        ...
        if (!res?.ok)
          throw res?.error || { code: 'INTERNAL_ERROR', message: 'No se pudo eliminar' };
      ...
      const msg =
        code === 'RESOURCE_CONFLICT'
          ? 'Operación en conflicto. Revisá los datos.'
...
function bindEventsOnce() {
  ...
  // Filtros
  qs('prd-cat').addEventListener('change', (e) => {
    state.categoryId = e.target.value || 'all';
    state.page = 1;
    reload();
  });
  qs('prd-status').addEventListener('change', (e) => {
  ...
  // Orden
  qs('prd-sort-by').addEventListener('change', (e) => {
    state.sortBy = e.target.value || 'name';
    reload();
  });
  qs('prd-sort-dir').addEventListener('change', (e) => {
    state.sortDir = e.target.value || 'asc';
    reload();
  });
  ...
  // Pager
  qs('prd-prev').addEventListener('click', () => {
    if (state.page > 1) {
      state.page--;
      reload();
    }
  });
  qs('prd-next').addEventListener('click', () => {
    state.page++;
    reload();
  });
...
export async function initProducts() {
  ...
  try {
    ...
    showSnackbar('No se pudieron cargar categorías', {
      type: 'error',
