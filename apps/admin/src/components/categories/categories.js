...
import { fetchCategories, getSnapshot, subscribe } from './categories.state.js';
import { renderCategoriesTable } from './categories.render.table.js';
import { attachCategoriesBindings } from './categories.render.bindings.js';
...
export async function refreshCategories(container) {
  try {
...
export async function initCategories(attempt = 0) {
  ...
  if (container.dataset.categoriesInit !== 'true') {
    attachCategoriesBindings(container, {
  ...
  try {
    await refreshCategories(container);
  } catch (err) {
    console.warn('[categories] initial refresh failed', err);
  }
}
