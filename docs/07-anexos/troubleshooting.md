...
- **Carrito/ofertas**: validar `discountPercent`; `finalPrice` es derivado.
...
| 409 `CATEGORY_IN_USE` / error FK    | Categoria asociada a productos segun `productCount`/relacion vigente. | Reasignar o eliminar productos asociados antes de borrar la categoria, segun politica vigente.          |
