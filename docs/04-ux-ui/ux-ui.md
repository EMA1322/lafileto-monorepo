...
- **Card** producto: imagen, titulo, precio base, `finalPrice` cuando hay `offer` activa y CTA.
...
- Columna **#Productos** muestra `productCount` cuando la API lo devuelve.
...
> NOTE: Client SPA usa botones por categoria consumidos desde API publica; mantener envelope y filtros sincronizados con `/api/v1/public/categories`.
