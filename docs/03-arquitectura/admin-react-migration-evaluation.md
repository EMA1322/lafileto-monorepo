...
> Actualizacion PR 8A cleanup: el Header global productivo ya es React y el
> Header legacy fue removido. El flujo `#not-authorized` sigue legacy.
...
- Header productivo se monta en React desde el router; ya no se carga por fetch de HTML ni `import()` legacy.
